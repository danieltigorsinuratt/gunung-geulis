export default function StatCard({ icon, value, label, badge, variant = 'default' }) {
    ...
    return (
        <div className={`rounded-xl border p-4 md:p-6 flex flex-col justify-between ${bgClass}`}>
            ...
            <div className="mt-3 md:mt-4">
                <div className={`text-3xl md:text-[40px] font-hanken font-bold leading-tight md:leading-12 ${valueClass}`}>
                    {value}
                </div>
                <div className={`text-[10px] md:text-xs font-mono font-medium uppercase tracking-wider mt-1 ${labelClass}`}>
                    {label}
                </div>
            </div>
        </div>
